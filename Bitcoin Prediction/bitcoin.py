...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0, len(Xi)):
        y= Xi.iloc[i]
        yi, b=  y[-1],Xi.iloc[i].iloc[0:(len(y) - 1)]
        meanofa = np.mean(a)
        meanofb = np.mean(b)
        #--------------Calculation of Similarity - EQ - 9 ---------------------
        M = len(a)
        sum = 0
        for j in range(0, M):
            sum += (a.iloc[j] - meanofa) * (b.iloc[j] - meanofb)
        similarity = sum/(M * np.std(a) * np.std(b))
        #----------Similarity calculation ends -------------------------
        eq6_n += yi * np.exp(wt * similarity)
        eq6_d += np.exp(wt * similarity)
    # formed eq6
    return eq6_n / eq6_d
